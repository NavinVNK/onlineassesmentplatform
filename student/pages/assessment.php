...
if (!isset($_SESSION['page_loaded'])) {
    ...
    // The page is being reloaded, perform your logout or other actions here
    $percent_score=0;
    $status="Refreshed Page";
    ...
    $result = $conn->query($sql);
        ...
        if ($result->num_rows > 0)
        {
            while ($row = $result->fetch_assoc())
            {
                $record_id=$row['record_id'];
                    $sql = "UPDATE tbl_assessment_records SET score='$percent_score', status='$status' WHERE record_id='$record_id'";
                if ($conn->query($sql) === TRUE) {
        ...
        VALUES ('$recid', '$myid', '$myname', '$exam_name', '$exam_id', '0', 'Refresh Page', '$next_retake', '$today_date')";
                    ...
                    <ul class="menu accordion-menu">
                      ...
                      <!--   <li><a href="students.php" class="waves-effect waves-button"><span class="menu-icon glyphicon glyphicon glyphicon-user"></span><p>Candidates</p></a></li> -->
                        <li><a href="examinations.php" class="waves-effect waves-button"><span class="menu-icon glyphicon glyphicon-book"></span><p>Assessments</p></a></li>
                         <li class="active"><a href="results.php" class="waves-effect waves-button"><span class="menu-icon glyphicon glyphicon-certificate"></span><p>Assessment Results</p></a></li>
...
if (sizeof($queslist) > 0)//$result->num_rows > 0)
{
    ...
    for ($x = 0; $x<sizeof($queslist); $x++)
    {
        ...
        if ($qno == "1")
        {
            print '<li style="background-color:'.$ques_style.';" role="presentation" class="active"><a href="#tab' . $qno . '" role="tab" data-toggle="tab"><span style="color:#000;">' . $qno . '</span></a></li>';
           // print '<div id="graystrip'. $qno.' "></div>';
        }
        else
        {
            print '<li style="background-color:'.$ques_style.';"role="presentation"><a href="#tab' . $qno . '" role="tab" data-toggle="tab"><span style="color:#000;">' . $qno . '</span></a></li>';
           // print '<div id="graystrip'. $qno.'"></div>';
        }
        ...
        if($qno>$ques_easy+$ques_medium+$ques_hard)
         ...
         print '<br><br><br><h5>Questions attended :</h5><div>';
