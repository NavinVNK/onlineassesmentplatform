...
$quest = 0;
$tflag=1;$eflag=1;$mflag=1;$hflag=1;
$sql = "SELECT * FROM tbl_questions WHERE exam_id = '$exam_id'";
$result = $conn->query($sql);
if ($result->num_rows >= $tot_ques) {
...
$result = $conn->query($sql);
$ques_easy=ceil($ques_easy_per*$result->num_rows)/100;
if ($result->num_rows >= $ques_easy) {
...
$result = $conn->query($sql);
$ques_medium=ceil($ques_medium_per*$result->num_rows)/100;
if ($result->num_rows >= $ques_medium) {
								     ...
								     if ($tflag == 0||$eflag == 0||$mflag == 0||$hflag == 0)
								                     {
                                                            ...
                                                            print '
                                                            <div class="alert alert-warning" role="alert">
                                                            The exam is not setup contact admin.
                                                            </div>';
                                                       }
                                                       else
                                                       {
